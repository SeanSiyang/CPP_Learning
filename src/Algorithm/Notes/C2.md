...
每个字节都有一个地址，计算机通过地址找到变量

变量的地址一般指它占用的字节中第一个字节的地址

指针就是变量的地址，指针是一个unsigned整数
...
**给指针变量赋值**的方式是把变量的地址取出来，赋给对应类型的指针变量
...
如果 ```p``` 存放了地址，```*p```是这个地址中存储的元素，如果修改```*p```，直接改变了这个地址所存放的元素

对于```int*```类型的指针变量```p```来说，```p+1```指```p```所指的int型变量的下一个int型变量地址

# 指针与数组

在数组元素前面加上取地址符```&```来获取元素的地址

```a + i``` 等同于 ```&a[i]```

```a + i```是数组a的首地址偏移i个int型变量的位置，是一个地址

要访问地址的元素，```*(a + i)```等价于```a[i]```
